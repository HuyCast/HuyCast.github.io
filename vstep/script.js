    ...
    state.questions.forEach((q, index) => {
        ...
        let html = `
            <div class="flex gap-4 mb-6">
                <div class="w-10 h-10 shrink-0 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center font-bold mt-1">
                    ${index + 1}
                </div>
        ...
        q.options.forEach(opt => {
            ...
            html += `
                <label for="${inputId}" class="option-label flex items-center p-4 rounded-xl border border-slate-200 bg-white/50 w-full group">
                    ...
                    <span class="text-slate-700 font-medium group-hover:text-slate-900 w-full break-words">${optTextWithImages}</span>
                </label>
            `;
        });
...
function showMotivationalFeedback(score) {
    ...
    if (score === 10) {
        ...
        styleClasses = "bg-amber-50/80 border-amber-200 text-amber-800";
        iconHTML = '<i class="ph ph-crown text-2xl md:text-3xl text-amber-500"></i>';
    } else if (score >= 8) {
        messages = excellentMessages;
        styleClasses = "bg-green-50/80 border-green-200 text-green-800";
        iconHTML = '<i class="ph ph-hands-clapping text-2xl md:text-3xl text-green-500"></i>';
    } else if (score >= 5) {
        messages = goodMessages;
        styleClasses = "bg-blue-50/80 border-blue-200 text-blue-800";
        iconHTML = '<i class="ph ph-thumbs-up text-2xl md:text-3xl text-blue-500"></i>';
    } else {
        messages = needsImprovementMessages;
        styleClasses = "bg-indigo-50/80 border-indigo-200 text-indigo-800";
        iconHTML = '<i class="ph ph-rocket-launch text-2xl md:text-3xl text-indigo-500"></i>';
    }
    ...
    feedbackEl.innerHTML = `
        <div class="mt-1 shrink-0 bg-white/60 p-2 rounded-lg border border-white shadow-sm flex items-center justify-center">
            ${iconHTML}
        </div>
        <div>
